...
use lilygo_epd47::{display::DisplayRotation, pin_config, Display, DrawMode};
use tinybmp::Bmp;
...
fn main() -> ! {
    ...
    let p = Text::with_alignment(
        text,
        display.bounding_box().center() + Point::new(0, -150),
        character_style,
        Alignment::Center,
    )
    .draw(&mut display)
    ...
    Image::new(&bmp_logo_rust, Point::new(50, p.y + 100))
        .draw(&mut display)
        .unwrap();

    Image::new(&bmp_logo_esphal, Point::new(300, p.y + 100))
        .draw(&mut display)
        .unwrap();
